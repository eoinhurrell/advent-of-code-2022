...
def update_positions(rope, movement):
    head = rope[-1][0].copy()
    tail = [x.copy() for x in rope[-1][1]].copy()
    head[0] += movement[0]
    head[1] += movement[1]

    tail = [move_towards(head_knot, tail_knot)
            for head_knot, tail_knot in zip([head] + tail, tail)]
    print(head, tail)
...
pos = list(map(lambda x: ",".join([str(y) for y in x[1][-1]]), positions))
